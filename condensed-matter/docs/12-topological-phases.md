...
最后要划清地界：本篇讲的全部能带拓扑（SSH、陈数绝缘体、$\mathbb Z_2$ 拓扑绝缘体）都是**短程纠缠**的态——拆掉对称性就能连续变形到平庸绝缘体，因此统称 SPT（symmetry-protected topological）。相互作用还能打开另一扇门：**内禀拓扑序**（长程纠缠、任意子、拓扑简并），分数量子霍尔态与自旋液体住那里——见补充材料[拓扑序：长程纠缠、规范结构与弦网](12s-topological-order.md)。
