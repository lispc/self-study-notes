...
这条"Jacob 阶梯"的隐喻：从 LDA 的"均匀气地狱"爬向"精确泛函天堂"，每上一阶多要一个变量、多一批参数。**必须清醒的一点**：与 CC 的 rank 阶梯不同（那里误差随截断 rank 系统性收敛、渐近可控），泛函阶梯**不保证系统性改进**——某个 GGA 在反应能上打败某个杂化是家常便饭；泛函的进步更像药物研发（针对病症调结构），不像数学截断（逼近论）。这是 DFT 与波函数方法在方法论气质上的根本差异，也是它被戏称为"部署机器"而非"推导机器"的原因。
...
**账本的读法**：DFT 的错误不是随机的，而是结构性的（能隙偏小、离域偏高、强关联失效）。加精确交换能缓解自相互作用与能隙问题（6.2 节的杂化解毒剂），但修复不了关联本身的结构性缺失——Mott 劈裂与长程色散不在任何单电子替身的射程内。知道错误长什么样，与知道答案同样重要。
...
**能隙**：$I = E(N-1) - E(N)$，$A = E(N) - E(N+1)$（电子亲合能），真能隙 $E_g = I - A = \mu^+ - \mu^-$（$\mu^\pm$ 是 $E(N)$ 在整数 $N$ 两侧的右/左导数）。但 KS 能隙用的是**同一个** $N$ 电子体系的两个本征值：

$$E_g^s = \varepsilon_{N+1}(N) - \varepsilon_N(N).$$

其中 $\varepsilon_N(N) = \mu^-$（上面的 HOMO 定理），而 $\varepsilon_{N+1}(N) \ne \mu^+$——LUMO 不是 $(N+1)$ 电子体系的 HOMO，差的那一段正是 $v_{xc}$ 在整数粒子数处的跳变，即**导数不连续性**：

$$\varepsilon_{N+1}(N) = \mu^+ - \Delta_{xc}\;\Longrightarrow\;E_g = \mu^+ - \mu^- = E_g^s + \Delta_{xc}.$$
