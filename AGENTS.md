...
QFT/量子力学/凝聚态/数字电路自学讲义仓库（"学习仓库"）。主要内容：根 `README.md`（书库入口）+ 四本书各自一个顶层目录（`quantum-mechanics/`、`qft-sm/`、`condensed-matter/`、`digital-design/`，各含 `README.md` 路线图与 `docs/` 笔记）+ `serve.py`（本地预览服务器）。笔记为简体中文，含 LaTeX 公式。

## 目录与文件约定

- 仓库 = 多本"书"并列：`<书名>/README.md` 是该书的路线图/目录，`<书名>/docs/` 是笔记。
- 量子力学书笔记路径：`quantum-mechanics/docs/NN-文档名.md`，平铺编号，与凝聚态书同风格，编号对应该书 README 目录的章号，留空位；氢原子一族为 06/06s/06s2/06s3。本书是 QFT 书与凝聚态书的共同上游。
- QFT 书笔记路径：`qft-sm/docs/stage-XX-主题/NN-文档名.md`，编号按路线图阶段内的学习顺序，留空位便于插入（如第 4 阶段 02–04 尚未写）。原 stage-00-math 与 stage-02-quantum-mechanics 的内容已独立成量子力学书，这两个 stage 目录不再存在。
...
- 新增笔记后必须同步该书 README：QFT 书在对应路线图条目末尾加 `（[笔记](docs/...)）` 链接；量子力学书、凝聚态书和数字电路书把目录条目改为链接。
- 跨书引用用相对路径（如凝聚态笔记引用 QFT 笔记：`../../qft-sm/docs/stage-XX/...`）。
- `serve.py` 是两层结构：`/` 书库首页只列书（`BOOKS` 字典，现有四本书），`/<书>/` 是该书目录页（按 `STAGES` 字典分节，平铺的书直接列文件）；新增书或 stage 目录时在对应字典加一行。
...
2. **表格单元格内数学不能含裸 `|`**。`|` 被当作列分隔符，会切烂 `$|0\rangle$`、`$|\mathcal M|^2$` 这类记号。用 `\lvert`、`\rvert`、`\lVert`、`\rVert` 代替。若原文是 `\left. ... \right|` 结构（如求值竖线），改 `\right|` 为 `\rvert` 时必须把配对的 `\left.` 一并删掉，否则 MathJax 报 "Extra \left or missing \right"。
